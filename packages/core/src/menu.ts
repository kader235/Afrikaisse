 ...
 * Le calcul du prix d'une ligne vit ici : le serveur le refait à chaque commande
 * (§20), et le menu client l'utilise pour afficher le même total.
 */
...
export const orderSchema = z.object({ ids: z.array(z.uuid()).min(1).max(500) });
...
/**
 * Prix unitaire = (prix promo ou prix) + variante + options. Refuse toute sélection que
...
export function priceLine(product: PricingProduct, selection: LineSelection): PricedLine {
  ...
  if (product.variants.length > 0) {
    if (!selection.variantId) return fail('VARIANT_REQUIRED', `Choisissez une taille ou une version pour « ${product.name} ».`);
